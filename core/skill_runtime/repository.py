...
def _evidence_to_dict(e: Evidence) -> dict[str, Any]:
    return {"evidence_id": e.evidence_id, "kind": e.kind, "title": e.title,
...
class SQLiteSkillRepository:
    ...
    def save_sprint(self, sprint: SkillSprint) -> None:
        ...
        with self._connect() as conn:
            conn.execute("""INSERT INTO skill_sprints
                ...
                state=excluded.state, hours_completed=excluded.hours_completed,
                evidence_json=excluded.evidence_json, assessment_json=excluded.assessment_json,
                version=excluded.version, updated_at=CURRENT_TIMESTAMP""",
                (sprint.sprint_id, sprint.skill_id, sprint.learner_id, sprint.state,
                 sprint.hours_completed, json.dumps(payload["evidence"]),
                 json.dumps(payload["assessment"]) if payload["assessment"] else None, sprint.version))
    ...
    def _row_to_sprint(row: sqlite3.Row) -> SkillSprint:
        ...
        s.state = row["state"]
        s.hours_completed = row["hours_completed"]
        ...
        if row["assessment_json"]:
            ...
            s.assessment = AssessmentResult(**a)
